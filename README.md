...
### バックエンドなしでのトポロジエディタ動作確認（モックモード）

`vite.config.mock.ts` に Vite のモックミドルウェアが定義されており、バックエンドなしで全 API レスポンスをローカルで代替する。

**前提条件**

- Node.js 18 以上（`nvm use 18` で切り替え可能）

**起動手順**

```bash
cd frontend
nvm use 18      # Node.js 18 に切り替え（初回のみ / nvm 使用環境の場合）
npm install     # 初回のみ
npm run dev:mock   # http://localhost:3000 で起動
```

ブラウザで `http://localhost:3000` を開くと、以下が利用可能になる。

| 機能 | モック時の動作 |
|------|----------------|
| 機器パレット表示 | Cisco / Juniper / Aruba / Fortinet の 5 テンプレートを表示 |
| ドラッグ&ドロップ配置 | キャンバスにノードを追加できる |
| ノード間リンク作成 | ノードのハンドルを繋いでリンクを作成できる |
| IP / VLAN / VRF 設定 | ノードを選択して設定フォームに入力できる |
| Config プレビュー | 各ノードのダミーコンフィグを Monaco Editor で確認できる |
| CML へ保存 | モック成功レスポンスを返す（実際には送信しない） |
| シミュレーション | バックエンド側で処理するため本機能は利用不可 |

> `npm run dev`（通常モード）はそのままバックエンドへのプロキシを使用するため、動作に変更はない。

**モックが代替するエンドポイント**

| エンドポイント | モックの動作 |
|---|---|
| `GET /api/v1/templates/` | 5 社 5 機種のテンプレートデータを返す |
| `GET /api/v1/topologies/` | 空リストを返す |
| `POST /api/v1/configs/generate` | ノードごとにダミーコンフィグを生成して返す |
| `POST /api/v1/configs/push-to-cml` | モック成功レスポンスを返す |
| `POST /api/v1/topologies/yaml-layout` | 空のノード/エッジを返す |
